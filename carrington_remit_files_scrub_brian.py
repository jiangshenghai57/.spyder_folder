...
# It will produce dat file and calculate the total net interest                                         #
...
# This library can open excel xlsx only. It cannot open old format of xls
...
# Example: last_mon(1809), returns 1808
#########################################################################################################
        ...
        try:
            ...
            if not os.path.exists(cur_mon_remit):
                ...
                # If there is more than two files and other than remit and remit scrub than ask user to choose 
                    ...
                    for i in range(0, len(remit_files)):
                        print("{}. {}".format(str(i), remit_files[i]))
            ...
            # load_workbook will take a bit of time, since it's process the entire remit file
            ...
            if not ws:
                ...
                print('You might want to check out the remit file and/or change the Worksheet\'s name.')
                print('Going to the next deal.')
                continue
            else:
                pass
            ...
            if not empty_row:
                ...
                for j in range(2, int(empty_row[1])):
                    ...
                    if tot_beg_bal == hist_row[0]:
                        ...
                        for j in range(2, int(empty_row[0])):
                            if is_empty(ws['G' + str(j)].value) == False:
                                tot_beg_bal = tot_beg_bal + ws['G' + str(j)].value
                ...
                if empty_row[-1] == int(row) - 2:
                    empty_row.pop()
                    for i in range(0, len(empty_row)):
                        for j in range(2, int(empty_row[i])):
                ...
                elif int(empty_row[0]) == int(empty_row[1]) - 1:
                    empty_row.pop()
                    for i in range(0, len(empty_row)):
                        for j in range(2, int(empty_row[i])):
                            if is_empty(ws['G' + str(j)].value):
            ...
            loan_range = range(2, int(stop_row))
            trans_bal = 0
            def_trans_bal = 0
            ...
            for i in range(0, len(hist_row2)):
            ...
            #######################################################################################
            # Overwrite hist-job is still a challenge, due to the fact hist-job is in xls format  #
            # xlwt is not working properly to overwrite the cells                                 #
            # May need another libraries for copying and pasting the cells                        #
            #######################################################################################
            ...
            # Getting the macro that takes the hist_row2 and paste into the hist job
            
            
            wb1 = xw.Book(hist_job_file)
            ...
            # Load the hist - job and ready to pasted the hist_row2
            # Maybe also add a beg_bal and end_bal check before check
            # have to use a different excel python library for xls (old excel format)
#            wb2 = xlrd.open_workbook(hist_job_file, formatting_info=True)
#            ws2 = wb2.sheet_by_name('g1data')
#            print(datetime.datetime(*xlrd.xldate_as_tuple(ws2.cell_value(1, 1), wb2.datemode)))
            # Find the end bal in the hist job that matches with the hist_row2[0] (beg_bal)
#            date_range = range(1, 360)
#            for i in date_range:
#                if (round(ws2.cell_value(i, 3), 2) - round(hist_row2[0], 2)) == 0:
#                    paste_row = i + 1
#                    break
#                elif isinstance(ws2.cell_value(i, 3), str) == True:
#                    break
#                else:
#                    paste_row = None
#                    continue

#            if is_empty(paste_row)==True:
#                print('Remit monthly beginning bal does not match with the ending bal in hist job!!')
#                print('Maybe check out the remit file.')
#                print('Onto the next deal.')
#                break

            # If paste_row found then past the hist_row2 in the hist job
            # later remittance total and other fees can be added at the at the end of the hsit job
            # coordination is like a matrix

#            ws2 = wb2.sheet_by_name('g1data')
#
#            for j in range(2, len(hist_row2)):
#                ws2.write(paste_row, j, round(hist_row2[j], 2))

#            wb2.save(hist_job_file)
#
        except:
